...
from apitest.src.utilities.genericUtilities import generate_random_coupon_code, generate_random_string
from apitest.src.helpers.coupons_helper import CouponsHelper
...
def test_create_coupon_percent_discount_type(my_setup, discount_type):
    ...
    # one of the tests is for not sending dicount type and verify the default is used,
    ...
    # verify the response
    assert rs_coupon_2['amount'] == pct_off, f"Create coupon with 50% off responded {rs_coupon_2['amount']} " \
            f"for amount. Expected: {pct_off}, Actual: {rs_coupon_2['amount']}."

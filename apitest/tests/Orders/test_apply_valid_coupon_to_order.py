...
def my_setup_teardown():
    # hard code 50% off coupon
    coupon_code = 'test50off'
    discount_pct = '50.00'

    # get random product for order
...
def test_apply_valid_coupon_to_order(my_setup_teardown):
    """
    Validate when coupon is applied to an order, the total amount is reduced by correct percentage
    """
    ...
    assert total == expected_total, f"Order total is not reduced after applying 50% coupon." \
        f" Expected cost: {expected_total}, Actual: {total}"
